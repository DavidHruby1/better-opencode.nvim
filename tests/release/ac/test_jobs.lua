...
T["AC-JOB-04 cancel removes one Job locally while the other continues"] = function()
  ...
  local client = fake_client({ abort = function(_, id)
  ...
  runtime.sessions.ses_a, runtime.sessions.ses_b = { id = "ses_a", active_job_key = a.key }, { id = "ses_b", active_job_key = b.key }
...
T["AC-JOB-05 cancel all snapshots every Runtime despite one abort failure"] = function()
  ...
  local client_a = fake_client({ abort = function()
    return Promise.reject({ error_class = "http" })
  end })
  local client_b = fake_client({ abort = function()
    return Promise.resolve({})
  end })
  ...
  runtime_a.sessions.ses_a, runtime_b.sessions.ses_b = { id = "ses_a", active_job_key = a.key }, { id = "ses_b", active_job_key = b.key }
...
T["AC-JOB-06 picker retains only owned Sessions and revalidates before reuse"] = function()
  ...
  eq(update.args[2].metadata, { client = "opencode.nvim-inline", contract_version = 2, root_hash = root_hash, last_mode = "build" })
...
T["AC-EVT-01 Runtime routes assistant parts by exact Session and Message identity"] = function()
  ...
  runtime:route_event({
    type = "message.updated",
    properties = { info = { id = "assistant_current", role = "assistant", sessionID = "ses_shared", parentID = "msg_current" } },
  })
  runtime:route_event({
...
T["AC-EVT-02 requests without messageID use the only active Job and fail closed otherwise"] = function()
  ...
  events.route(runtime, { type = "question.asked", properties = { sessionID = session.id, requestID = "question-1", questions = {} } })
  ...
  runtime.jobs[permission.key], runtime.sessions[permission.session_id] = permission, {
  ...
  eq({ permission.state, permission.waiting_kind, permission.waiting_request_id }, { "waiting_user", "permission", "permission-1" })
  ...
  events.route(orphan, { type = "permission.asked", properties = { sessionID = "ses_missing", requestID = "permission-1", permission = "read" } })
...
T["AC-INT-01 question reply targets one request and resumes only its Job"] = function()
  ...
  runtime.sessions.ses_b, runtime.sessions.ses_a = { id = "ses_b", active_job_key = job.key }, { id = "ses_a", active_job_key = other.key }
  local old_select = vim.ui.select
  vim.ui.select = function(items, _, callback)
    callback(items[1])
  end
...
T["AC-INT-02 permission UI exposes supported actions and hard-denies write capability"] = function()
  ...
  denied_runtime.jobs[denied.key], denied_runtime.sessions.ses_denied = denied, { id = "ses_denied", active_job_key = denied.key }
  events.route(denied_runtime, {
    type = "permission.asked",
    properties = { sessionID = "ses_denied", requestID = "permission_edit", permission = "edit" },
  })
  eq({ denied.state, denied.error_class, denied_runtime.prompt_locked, denied_runtime.reconciliation_required },
...
T["AC-INT-03 dialogs remain FIFO while waiting Jobs and conflicts keep their states"] = function()
  ...
  events.route(runtime, { type = "permission.asked", properties = { sessionID = "ses_permission", requestID = "p", permission = "webfetch" } })
  require("opencode.interaction").enqueue({ root = root, session_id = "ses_conflict", job_key = conflict.key, kind = "agent_conflict" })
  local interaction = require("opencode.interaction")
  interaction.advance()
  eq({ interaction.current.job_key, interaction.queue[1].job_key, interaction.queue[2].job_key },
  ...
  events.route(runtime, { type = "permission.rejected", properties = { sessionID = "ses_permission", requestID = "p" } })
  eq(vim.wait(1000, function()
...
T["AC-INT-04 canonical reply restores sidebar visibility without unlocking TUI input"] = function()
  ...
  events.route(runtime, {
    type = "question.asked",
    properties = { sessionID = job.session_id, requestID = "req_once", questions = { { options = { { label = "yes" } } } } },
  })
  local interaction = require("opencode.interaction")
  interaction.advance()
  eq(vim.wait(1000, function()
  ...
  eq({ runtime.interaction_locked, runtime.prompt_locked, runtime.sidebar.visible, runtime.sidebar.hide_calls }, { true, true, false, 1 })
  eq(runtime.sidebar.input_locked, true)
  eq(replies, { { id = "req_once", answers = { { "yes" } } } })
  events.route(runtime, { type = "question.replied", properties = { sessionID = job.session_id, requestID = "req_once" } })
  eq({ runtime.interaction_locked, runtime.prompt_locked, runtime.sidebar.visible, runtime.sidebar.show_calls }, { false, false, true, 1 })
...
T["AC-STATE-01 Job transition matrix accepts only declared transitions and required kinds"] = function()
  local jobs = require("opencode.job")
  local states = { "running", "waiting_user", "pending_apply", "conflict", "completed", "cancelled", "error", "scope_violation" }
  local allowed = {
    running = { waiting_user = true, pending_apply = true, completed = true, cancelled = true, error = true, scope_violation = true },
...
T["AC-STATE-02 Session availability follows local Job state before remote status"] = function()
  ...
  for _, state in ipairs(active_states) do
    ...
    runtime.jobs[job.key], runtime.sessions[session.id] = job, session
    eq({ (sessions.availability(runtime, session, "idle")), (sessions.availability(runtime, session, "idle")) }, { "active", "active" })
